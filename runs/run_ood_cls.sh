...
for n_ood_cls in `seq 100 20 180`;    #(100,120,140,160,180);
do
    n_cls=$((n_total - n_ood_cls))
    ID_args="--dataset "$id_data" --num_classes "$n_cls
    OOD_args="--ood "$ood" --num_ood_class "$n_ood_cls
    tc_save_name="M:"$method"_arch:$tc""_ID:"$id_data"_ic:"$n_cls"_trial:"$trial
    ...
    python evaluate.py --path $ROOT_DIR/student/model/$save_name/$arch"_last.pth" --num_classes $min_n_cls
done
